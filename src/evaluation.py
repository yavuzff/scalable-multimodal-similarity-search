...
# Exact index params: modalities, dimensions, distance metrics, weights

def evaluate_insertion(index, entities: list[np.ndarray]):
    """
    Evaluate the time it takes to add entities to the index.
    """
    pass


class Params:
    def __init__(self,modalities: int, dimensions: list[int], metrics: list[str], weights: list[float], dataset: list[np.ndarray], index_size: int, k: int, query_ids: list[int]):
        ...
        if self.index_size > len(dataset[0]):
            print(f"Warning: requested index size {self.index_size} < dataset size ({len(dataset[0])}. Setting index_size to {len(dataset[0])}")
...
# next: evaluate and save HNSW results.
